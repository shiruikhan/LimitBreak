...
- **Design system:** `utils/design_system.py` → `inject_design_system(page_variant)` injeta CSS global (variáveis `--color-lime`, `--bg-card`, etc., fontes Bebas Neue/Space Grotesk/JetBrains Mono, estilização de botões/inputs/tabs/sidebar, fade-in de página, media queries mobile ≤640px); chamado uma vez em `app.py`. Helpers: `render_page_heading(title, subtitle, tone=lime|gold|red|ember|purple|blue)`, `coin_badge(value)`, `stat_tile(label, value, tone=...)` (envolver em `<div class='lb-stat-row'>`), `render_empty_state(icon, title, body)`, `render_evolution_animation(from_img, to_img, from_name, to_name)` (banner animado compartilhado — usado em equipe/calendario/treino). Classes compartilhadas: `lb-card`, `lb-panel`(+`-title`/`-sub`), `lb-hero`(+`-title`/`-sub`), `lb-stat-tile`, `lb-progress` (`<span>` interno com tom `blue|purple|gold|red`), `lb-chip`, `lb-badge`, `lb-banner` (tons `success|info|warning|danger|purple|lime`), `lb-empty-state`, `lb-skeleton`. **Não duplicar CSS de paleta/fonte/título em páginas** — todas as 18 páginas já consomem o design system (migração concluída em junho/2026)
...
- **Design system global (`utils/design_system.py`):** CSS vars, fontes e componentes (`render_page_heading`, `coin_badge`, `stat_tile`, `render_empty_state`, `render_evolution_animation`); injetado em `app.py`. **Migração completa em junho/2026** — todas as páginas usam as classes `lb-*`; CSS local restante é só de layout específico da página. Inclui responsividade mobile (≤640px), fade-in de página e microinterações (hover lift, pulso no VS da arena, wiggle de ovo pronto)
